...
INT8 ir_ac_lib_parse()
{
    ...
    {
		Serial.printf("binary_parse_offset failed\n");
        return IR_DECODE_FAILED;
    }

    if (IR_DECODE_FAILED == binary_parse_len())
    {
		Serial.printf("binary_parse_len failed\n");
        return IR_DECODE_FAILED;
    }

    if (IR_DECODE_FAILED == binary_parse_data())
    {
		Serial.printf("binary_parse_data failed");
        return IR_DECODE_FAILED;
    }
    ...
    for (i = 0; i < tag_count; i++)
    {
        ...
        if (context->si.type == SWING_TYPE_NORMAL)
        {
            ...
            if (tags[i].tag == TAG_AC_SWING_1)
            {
                ...
                {
					Serial.println("context->swing1.comp_data == NULL");
                    return IR_DECODE_FAILED;
                }
                ...
                if (IR_DECODE_FAILED == parse_common_ac_parameter(&tags[i],
                                                                  ...
                                                                  AC_PARAMETER_TYPE_1))
                {
					Serial.println("parse_common_ac_parameter failed");
                    return IR_DECODE_FAILED;
                }
            }
            else if (tags[i].tag == TAG_AC_SWING_2)
            {
                ...
                {
					Serial.println("context->swing2.comp_data == NULL");
                    return IR_DECODE_FAILED;
                }
                ...
                if (IR_DECODE_FAILED == parse_common_ac_parameter(&tags[i],
                                                                  ...
                                                                  AC_PARAMETER_TYPE_2))
                {
					Serial.println("parse_common_ac_parameter2 failed");
                    return IR_DECODE_FAILED;
                }
            }
        }
        ...
        else if (tags[i].tag == TAG_AC_FUNCTION_2)
        {
            ...
            {
				Serial.println("parse_function_2_tags34 failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_FRAME_LENGTH)
        {
            ...
            {
				Serial.println("parse_frame_len failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_ZERO)
        {
            ...
            {
				Serial.println("parse_zero failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_ONE)
        {
            ...
            {
				Serial.println("parse_one failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BOOT_CODE)
        {
            ...
            {
				Serial.println("parse_boot_code failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_REPEAT_TIMES)
        {
            ...
            {
				Serial.println("parse_repeat_times failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BIT_NUM)
        {
            ...
            {
				Serial.println("parse_bit_num failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_ENDIAN)
        {
            ...
            {
				Serial.println("parse_endian failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BAN_FUNCTION_IN_COOL_MODE)
        {
            ...
            {
				Serial.println("parse_nmode N_COOL failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BAN_FUNCTION_IN_HEAT_MODE)
        {
            ...
            {
				Serial.println("parse_nmode N_HEAT failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BAN_FUNCTION_IN_AUTO_MODE)
        {
            ...
            {
				Serial.println("parse_nmode N_AUTO failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BAN_FUNCTION_IN_FAN_MODE)
        {
            ...
            {
				Serial.println("parse_nmode N_FAN failed");
                return IR_DECODE_FAILED;
            }
        }
        else if (tags[i].tag == TAG_AC_BAN_FUNCTION_IN_DRY_MODE)
        {
            ...
            {
				Serial.println("parse_nmode N_DRY failed");
                return IR_DECODE_FAILED;
            }
        }
    }
    ...
    for (i = 0; i < tag_count; i++)
    {
        ...
        if (tags[i].tag == TAG_AC_DELAY_CODE)
        {
            ...
            {
				Serial.println("parse_delay_code failed");
                return IR_DECODE_FAILED;
            }
        }
        if (tags[i].tag == TAG_AC_LAST_BIT)
        {
            ...
            {
				Serial.println("parse_lastbit failed");
                return IR_DECODE_FAILED;
            }
        }
    }
...
#endif
	Serial.println("try to malloc ir_hex_code");
	Serial.printf("context->default_code.len = %d\n", context->default_code.len);
    ir_hex_code = (UINT8 *) ir_malloc(context->default_code.len);
    if (NULL == ir_hex_code)
    {
        // warning: this AC bin contains no default code
		Serial.println("ir_hex_code == NULL");
        return IR_DECODE_FAILED;
	}
	else {
		Serial.println("ir_hex_code is not null\n");
	}
...
{
	Serial.println("free_ac_context");
    UINT16 i = 0;
