...
*/
boolean downLoadFile(String index_id);
...
void callback(char* topic, byte* payload, unsigned int length) {
	...
	if ((topic_str == (topic_head + mode_set)) && (cmd == "off")) {
...
boolean downLoadFile(String index_id) {
	...
	HTTPClient http;
	String download_url = "http://irext-debug.oss-cn-hangzhou.aliyuncs.com/irda_"  + index_id + ".bin";
	boolean flag = false;

	if (WiFi.status() == WL_CONNECTED) {

		// Prepare cache file
		String filename = index_id;
		...
		if (!cache) {
			...
			SERIAL_DEBUG.println(download_url);
